...
if __name__ == "__main__":
    ...
    model = Unet(in_channels=4, n_classes=5, n_layers=2, n_nodes=3) # Unet for Brats
    ...
    if args.v1:
        ...
        trainer = DartsTrainer(model,
                               ...
                               callbacks=[LRSchedulerCallback(lr_scheduler), ArchitectureCheckpoint(
                                   "./final architecture/checkpoints")])
        ...
        json.dump(trainer.export(), open('./final architecture/checkpoint_{}'.format(time.strftime('%Y%m%d-%H%M%S'))+'.json', 'w'))
