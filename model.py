...
class Unet(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, n_layers, n_nodes=4,
        ...
        down_outc = 4 * mid_channel  # same to downoutchannel
        channels_c = down_outc * stem_multiplier
        channels_pp, channels_p, channels_c = channels_c, channels_c, mid_channel

        downcs_nfilter = []
        downcs_nfilter.append(channels_p)
        downcs_nfilter.append(channels_pp)

        self.stem = nn.Conv2d(down_outc, channels_pp, kernel_size=1)
        ...
        for i in range(n_layers):
            ...
            channels_c = 2 * channels_c
            down_cell = Cell(n_nodes, channels_pp, channels_p, channels_c, reduction_p, reduction)
            self.down_cells.append(down_cell)
            channels_pp, channels_p = channels_p, channels_c * n_nodes
            downcs_nfilter.append(channels_p)

        # create up cells
        reduction_p, reduction = reduction, False
        for i in range(n_layers):
            channels_p = channels_c * n_nodes
            channels_c = channels_c // 2
            channels_pp = downcs_nfilter[-(i+2)]
            up_cell = UpCell(n_nodes, channels_pp, channels_p, channels_c, reduction_p, reduction)
            self.up_cells.append(up_cell)

        self.seblock = ops.SEBasicBlock(64, 64)
        self.gap = nn.AdaptiveAvgPool2d(1)
